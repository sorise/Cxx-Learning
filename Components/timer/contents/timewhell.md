...
#include <type_traits>
#include <iomanip>
#include "include/timer/WheelTimer.hpp"

using namespace std::chrono_literals;

//248.5513481
int main() {
    muse::timer::TimerWheel timer;

    std::cout << "Add "<< muse::timer::TimerWheel::GetTick().count() <<std::endl;
    std::shared_ptr<muse::timer::TimerWheelTask> task1 = timer.setTimeout(1000ms, [](const std::string& name){
        std::cout << "Run DateTime: " << muse::timer::TimerWheel::GetTick().count() <<std::endl;
    }, "task no.1");

    std::cout << "Add "<< muse::timer::TimerWheel::GetTick().count() <<std::endl;
    std::shared_ptr<muse::timer::TimerWheelTask> task2 = timer.setTimeout(1500ms, [](const std::string& name){
        std::cout << "Run DateTime: " << muse::timer::TimerWheel::GetTick().count() <<std::endl;
    }, "task no.2");

    std::shared_ptr<muse::timer::TimerWheelTask> task3 = timer.setTimeout(2000ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.3");

    timer.clearTimeout(task3);

    std::shared_ptr<muse::timer::TimerWheelTask> task4 = timer.setTimeout(2010ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.4");

    std::shared_ptr<muse::timer::TimerWheelTask> task5 = timer.setTimeout(5000ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.5");

    std::shared_ptr<muse::timer::TimerWheelTask> task6 = timer.setTimeout(5100ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.6");

    std::shared_ptr<muse::timer::TimerWheelTask> task7 = timer.setTimeout(5200ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.7");

    std::shared_ptr<muse::timer::TimerWheelTask> task8 = timer.setTimeout(10000ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.8");

    std::shared_ptr<muse::timer::TimerWheelTask> task9 = timer.setTimeout(12000ms, [](const std::string& name){
        std::cout <<"Running the " << name << std::endl << std::flush;
    }, "task no.9");
