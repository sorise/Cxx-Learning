...
**2. 编译** 编译简单来讲，就是将预处理过后的“纯C++语言”转化成汇编语言。使用预处理的输出结果作为输入，**生成平台相关的汇编代码(assembly code)**。结果是文本格式，文件以.s结尾。
...
这些编译模式可以根据项目需求和开发目标进行选择，并且可以在构建系统中进行配置。每种模式都有其适用的场景和权衡，开发者可以根据具体情况选择最合适的编译模式。
